...
auto hash_combine = [](cauto &x, cauto &y) noexcept {
  ...
  return origin ^
         (hash{}(x.exit) + 0x517cc1b727220a95 + (origin >> 2) + (origin << 6));
};
auto subrange(auto pair) {
  return ranges::subrange{get<0>(pair), get<1>(pair)};
}
template <class Path>
  requires same_as<remove_cvref_t<Path>, path>
void create_file(Path &&p) {
  ofstream{p};
}
} // namespace utilities
...
template <class... Functors> struct Visitor : Functors... {
